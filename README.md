...
**Откуда пришёл:** `shop.html` пришёл из **двух мест**:

1. **Из `index.html`** (Главная страница) — через футер (ссылка на каталог или раздел товаров)
2. **Из `faq.html`** (Terms & Conditions) — тоже через футер или другие ссылки

Также в вашей таблице тулбара он указан как **«Сын»** и приходит из `msi-prestige.html` (MSI Prestige Series) через `Forward →`.
...
| HTML файл | Страница (H1) | Кому надо  <br>< Back | Кому надо <br>Forward > | Родня | Заголовок `<h1>` |
|-----------|---------------|------------------|---------------------|-------|----------------|
